...
- [ ] 初始化 Python 项目结构 (pyproject.toml, src/, tests/)
- [ ] 实现配置管理模块 (读取/写入 ~/.p2r_config.json)
- [ ] 实现 MinerU API 客户端
  - [ ] 文件上传链接申请
  - [ ] 文件上传 (PUT)
  - [ ] 任务状态轮询
  - [ ] ZIP 结果下载与解压
- [ ] 添加基础 CLI 命令 (`p2r convert`)
- [ ] **测试点**：手动测试不同类型 PDF 的解析效果
...
- [ ] 实现命名模板引擎
- [ ] 实现 Frontmatter 生成与注入
- [ ] 实现文件移动逻辑 (临时目录 → 目标目录)
- [ ] 实现重复文件版本号递增
- [ ] 实现源 PDF 原地重命名

### Phase 3: PicGo 集成与回退机制
